...
class AgentState:
    ...
    def __hash__(self):
        return hash((self.configuration, self.scared_timer))
...
class GameStateData:
    ...
    def __str__( self ):
        ...
        for agent_state in self.agent_states:
            ...
            if agent_state.configuration == None: continue
...
class GameState(GameStateData):
    """
    A wrapper around GameStateData to provide methods like
    get_walls(), get_num_food(), generate_successor(), etc.
    """

    def __init__(self, prev_state=None):
        if prev_state:
            self.data = prev_state.deep_copy()
        else:
    ...
    def get_num_food(self):
        return self.data.get_num_food()

    def generate_successor(self, agent_index, action):
        # call the original GameStateData method
        new_data = self.data.generate_successor(agent_index, action)
        return GameState(new_data)
